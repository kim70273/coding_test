...
// 오류가 나는 것을 알게 되었습니다.
// 수정해야 할 소스 파일이 너무 많아서 고민하던 "콘"은 소스 코드에 작성된 모든 괄호를 뽑아서 올바른 순서대로 
...
function solution(p) {
    ...
    for(let i=0;i<arr.length;i++){
        ...
        if(leftCount === rightCount){
            result.push(...arr.slice(point,i+1))
            point=i+1;
            leftCount=0;
            rightCount=0;
        }
        
        //result에 담긴것이 올바른 문자열인지 판단.
        const d = [];
        result.forEach(e => {
            if(e==='(') d.push(e);
            else d.pop();
        })
        if(d.length>0){//올바른 문자열이 아닐경우
            answer.push('(');
            temp.push([...result.splice(1,result.length-2)]);
        }else{
            answer.push(...result);
        }
    }

...
// 친구들로부터 천재 프로그래머로 불리는 "프로도"는 음악을 하는 친구로부터 자신이 좋아하는 노래 가사에 사용된 단어들 중에 특정 키워드가 
// 몇 개 포함되어 있는지 궁금하니 프로그램으로 개발해 달라는 제안을 받았습니다.
...
function solution(words, queries) {
    ...
    queries.forEach(w =>{
        ...
        for(let i=0;i<words.length;i++){
            if(head === 10000 & tail === 10000){
                if(words[i].length === len) count ++;
                continue;
            }
            
            if(words[i].length === len){
                if(words[i].substring(head,tail+1)===w.substring(head,tail+1)) count++
            }
        }
        answer.push(count);
    })

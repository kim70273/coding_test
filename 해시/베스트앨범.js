...
function solution(genres, plays) {
    ...
    for(let key in album){
        let sum = 0;
        album[key].forEach((play) => {
            sum+=play[0];
        })
        rank.push([key,sum]);
    }//장르에서 가장 많이 들은 것이 아닌 장르의 총합으로 해야 됨.
    rank.sort((a,b) => b[1]-a[1]);
    
    for(let i=0;i<rank.length;i++){
        if(album[rank[i][0]].length>1){
            answer.push(album[rank[i][0]][0][1]*1);
            answer.push(album[rank[i][0]][1][1]*1);
        }
        else{
            answer.push(album[rank[i][0]][0][1]*1);
        }

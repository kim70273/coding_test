...
// 오른쪽에는 바구니가 있습니다. (위 그림은 "5 x 5" 크기의 예시입니다). 각 격자 칸에는 다양한 인형이 들어 
// 있으며 인형이 없는 칸은 빈칸입니다. 모든 인형은 "1 x 1" 크기의 격자 한 칸을 차지하며 격자의 가장 아래 칸부터 차곡차곡 쌓여 있습니다. 
...
// 배열 moves가 매개변수로 주어질 때, 크레인을 모두 작동시킨 후 터트려져 사라진 인형의 개수를 return 하도록 solution 함수를 완성해주세요.
...
function solution(board, moves) {
    ...
    for(let i=0; i<moves.length;i++){
        ...
        for(let j=0; j<board.length;j++){
            if(board[j][target] !== 0){
                if(stack.length>0 && stack[stack.length-1]===board[j][target]){
                    stack.pop();
                    answer+=2;
                    board[j][target]=0;
                }else{
